...
extern "C" void writensc(char* slabel, int nsc[], int isbulk, int ia1, int ia2);
/*
 ...
 * 
 *    char* slabel : systemLabel
 *    int[3] nsc    : number of cells along each direction
 *    bool  isbulk : lead or EM
 *    int ia1 :    FC calc. first
 *    int ia2 :   FC calc. last
 * */
void writensc(char* slabel_, int nsc[], int isbulk, int ia1, int ia2)
{
...
/* Read kpoints */
    ...
    int nkp = 1;
    vector<vector<double> > vkp = {{0.0, 0.0, 0.0, 1.0}};
...
//    ofc.write((char*)&xyz[0][0], sizeof(double)*(na*3));
    ...
    ofc.write(reinterpret_cast<char*>(xmass), sizeof(xmass));
    ofc.write(reinterpret_cast<char*>(nsc), sizeof(int)*3);
...
//    ofc.write((char*)&scell[0][0], sizeof(double)*9);
    ...
    if(!isbulk){
        std::string ssgflft("bulksgf.lft");
        std::string ssgfrt("bulksgf.rt");
        std::fstream sfsl(ssgflft, std::ios::in|std::ios::binary);
        std::fstream sfsr(ssgfrt, std::ios::in|std::ios::binary);
        if(!sfsl || !sfsr){
            std::cerr << "Error when open " << ssgflft << " and " << ssgfrt << std::endl;
            abort();
        }
        double cellLz_l;
        double cellLz_r;
        int naAtOnelayer_l;
        int naAtOnelayer_r;
        sfsl.read((char*)&cellLz_l, sizeof(double));
        sfsr.read((char*)&cellLz_r, sizeof(double));
        sfsl.read((char*)&naAtOnelayer_l, sizeof(int));
        sfsr.read((char*)&naAtOnelayer_r, sizeof(int));
        if(ia1 == 1 && ia2 == na){
           double minxaz = xyz[0][2];
           for(size_t i = 1; i < na; ++i) {
               if(xyz[i][2] < minxaz) minxaz = xyz[i][2];
           }
           ia1 = na + 1, ia2 = -1;
           for(size_t i = 0; i < na; ++i) {
               if(fabs(xyz[i][2] - minxaz - cellLz_l) < 1.0e-2){
                    if(i < static_cast<size_t>(ia1)) ia1 = i;
               }
               if(fabs(xyz[i][2] - minxaz - (cell[2][2] - cellLz_r)) < 1.0e-2){
                    if(i > static_cast<size_t>(ia2)) ia2 = i;
               }
           }
           if(ia1 > na || ia2 < 0){
               std::cerr << "Error: too small cells" << std::endl;
               abort();
           }
        }
        else {
            ia1 = ia1 - naAtOnelayer_l;
            ia2 = ia2 + naAtOnelayer_r;
            if(ia1 < 0 || ia2 > na){
                std::cerr << "Error: too small cells" << std::endl;
                abort();
            }
        ...
        sfsl.close();
        sfsr.close();
        ofc.write((char*)&ia1, sizeof(int));
        ofc.write((char*)&ia2, sizeof(int));
    }
...
//    ifc.read((char*)xmassr, sizeof(int)*nar);
    ...
    ifc.read((char*)xmassr, sizeof(xmassr));
    cout << "xyz xmass:" << endl;
    for(int i = 0; i < nar; i++){
        for(int j = 0; j < 3; j++)
            cout << xyzra[i][j] << "    ";
    ...
    cout << "nsc:"<<endl;
    int nscr[3];
    ifc.read((char*)nscr, sizeof(nscr));
    for(int j = 0; j < 3; j++)
        cout << nscr[j] << " ";
    cout << endl;
//
...
/* Write MD optins: Force Constant Calculations*/
    ...
    if(isbulk){
        ...
        ofs << "IsBulkFC              F" << endl;
        ofs << "MD.FCfirst            " << ia1-1+i1 << endl;
        ofs << "MD.FClast            " << ia2-1+i1 << endl;
    }
