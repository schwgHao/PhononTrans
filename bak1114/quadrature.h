...
struct Trapzd : Quadrature {
	...
	Doub next() {
		Doub x,tnm,sum,del;
		Int it,j;
		n++;  // H. Wang: here add self to control integral precision.
		if (n == 1) {
			return (s=0.5*(b-a)*(func(a)+func(b)));
		} else {
			for (it=1,j=1;j<n-1;j++) it <<= 1;//double it
			tnm=it;
			del=(b-a)/tnm;
			x=a+0.5*del;
...
Doub qtrap(T &func, const Doub a, const Doub b, const Doub eps=1.0e-10) {
	...
	for (Int j=0;j<JMAX;j++) {
		s=t.next();//H.Wang: JMAX determines the value of n
//		cout << "s-olds	" << fabs(s-olds) << endl;
//		cout << "olds	" << fabs(olds) << endl;
//		cout << "j " << j << endl;
		if (j > 5)
		...
		if ((j > 20) && (j%10 == 0))
		{
			cout << "Step: " << j << " olds: " << olds;
			cout << " dInt: " << fabs(s-olds)/fabs(olds) << endl;
		}
	}
