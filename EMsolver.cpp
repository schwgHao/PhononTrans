...
void emsolver::DRCC(vector<vector<vector<vector<complex<double> > > > >& dccr, vector<vector<complex<double> > >& VibTRCOverk, vector<vector<vector<vector<complex<double> > > > >& SelfEngLeadL, vector<vector<vector<vector<complex<double> > > > >& SelfEngLeadR, const vector<vector<vector<vector<complex<double> > > > >& d00rl, const vector<vector<vector<vector<complex<double> > > > >& d00rr)
{
...
//        std::cout << "KLm: " << KLm << std::endl;
        ...
        for(int o = 0; o < omg.size(); o++){
            ...
            ofs << std::scientific << hbar * unitconv * 1000.0 * omg[o] << "        " << -2.0/pi / (unitconv * hbar * 1000.0)  *  omg[o] * D00l.trace().imag() << "        ";
...
//            Eigen::MatrixXcd TC = (Iden - KC - SelfEngL - SelfEngR).fullPivLu().inverse();
            ...
            ofs << std::scientific <<  -2/pi / (unitconv * hbar * 1000.0)  *  omg[o] * TC.trace().imag() << std::endl;
