...
void surfphGF::DR00(v4cd& d00r){
    ...
    std::cout << string(60,'*')<<std::endl;
    std::cout << "SurfphGF: Solve surface phonon green's function iteratively." << std::endl;
    int nk = mfcbylz.size();
    int no = omgv.size();
...
//    std::cout << "nx: " << nx << std::endl;
    for(int k = 0; k < nk; k++){
        ...
        for(int o = 0; o < no; o++){
            ...
            std::cout << "omega " << o << "/" << no << " : " << omgv[o] << std::endl;
            ...
            bool lconv = DeciTech(mM, mMpre, mFpre, mq, vFrq, alpha, exFreq, nStep, 0);
            if(lconv){
                K0s = mM["K0s"];
            }
...
bool surfphGF::DeciTech(map<string, MatrixXcd>& mM, map<string, MatrixXcd> mMpre, map<string, MatrixXcd> mFpre, map<string, std::pair<queue<MatrixXcd>, queue<MatrixXcd> > >& mq, const MatrixXcd& vFrq, const double alpha, const size_t exFreq, const size_t nStep, size_t step)
{
    ...
    double meps = std::max(K1.norm(), K2.norm());
...
//    }
    ...
    if(meps > 1.0e20) {
        std::cout << "warning: step " << step << ", deps = " << meps;
        std::cout << ", can not converge to finite values!" << std::endl;
        return false;
 //       abort();
    }

    if(meps < 1.0e-8) {
        std::cout << std::scientific << "step " << step + 1 << " :  d eps = " << meps << std::endl;
        return true;
    }

    if((step+1) % 100 == 0){
        std::cout << "step, d eps: " << step + 1 << " : "<< meps << std::endl;
    }
...
//    MatrixXcd fk0s = K0sNext;
    ...
    const size_t sq = mq["K0s"].first.size();
...
//    DeciTech(mM, mMpre, mFi, K2, Iden);
    DeciTech(mM, mMpre, mFi, mq, vFrq, alpha, exFreq, nStep, step+1);
}
