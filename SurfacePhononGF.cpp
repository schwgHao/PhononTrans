...
//    std::cout << "nk " << nk << std::endl;
    ...
    for(int k = 0; k < nk; k++){
        ...
        for(int o = 0; o < no; o++){
            ...
            std::cout << o << " of " << no << " : at omega = " << omgv[o];
...
//    }
    ...
    if(meps > 1.0e20) {
        std::cout << "Stop at step " << step << " , deps = " << meps << std::endl;
        std::cerr << "Error: can not converge to finite values!" << std::endl;
        abort();
    }

    if(meps < 1.0e-8) {
        std::cout << std::scientific <<  ", d eps = " << meps << ", converged after " << step + 1 << " step." << std::endl;
        return;
    }
...
void surfphGF::writeDR00(string ssgf, double cellLz, int naAtOnelayer, const vector<vector<double> >& kp2d, const v4cd& d00r)
{
